...
|------------|-------------|
| `Fully`    | Intent has **full** control over the particular _syntax node_, any deviations in the existing file's _syntax node_ are overwritten with the content generated by the template. Descendent _syntax nodes_ can be opted-out of being fully managed having an `[IntentManaged]` attribute applied to them. |
| `Merge`    | Intent may make additions to _the syntax node_ but is not allowed to remove any existing content from it. |
| `Ignore`   | Intent must **ignore** this _syntax node_ and not remove or overwrite it with content generated by the template. Code management instructions on descendent _syntax nodes_ are likewise ignored, i.e. it is not possible to opt-out of being ignored as a descendent. |
...
_Type declaration syntax nodes_ (classes, enums, interfaces, etc) and _type member declaration syntax nodes_ (fields, methods, properties, etc) can have an `[IntentManaged]` attribute applied to them to control their code management behaviour.
...
> [!NOTE]
> When a _syntax node_ has `[IntentManaged(Mode.Ignored)]` applied to it, descendent nodes' code management instructions are also ignored and disregarded, i.e. it is not possible to opt-out of being ignored as a descendent.
