...
When a _type declaration syntax node_ has `[IntentManaged(Mode.Ignored)]` applied to it, descendent nodes (i.e. members, such as fields, properties, etc.) all become ignored and will stop being updated or removed. Additionally, code management instructions on descendent nodes are also ignored and disregarded meaning it is not possible to "opt-out" of being ignored as a descendent.
