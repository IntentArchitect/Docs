...
- Fixed: The [Software Factory CLI](xref:tools.software-factory-cli) would return a `0` return code even when an exception occurred during Software Factory execution.
...
### Issues fixed in 3.3.14

- Fixed: The Software Factory would throw an exception about a missing path when a module contained no `lib` folder, for example for modules generated by packing an Intent Architect application.
- Fixed: Changing `Is Collection` or `Is Nullable` on an element would not trigger any `On Type Changed` events for it.
