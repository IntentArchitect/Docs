...
5.1 added Git source control and 5.2 added Changes Review, so you could see what was about to land in your repository. Both stopped at the commit boundary - once a change left Intent Architect as a pull request, reviewing, discussing and merging it meant going to a browser. 5.3 closes that gap: pull requests can now be created, reviewed and merged from inside Intent Architect, on both GitHub and Azure DevOps.

The other three areas of focus this release each came from a specific gap:

- **Spec-Driven Development traceability was self-reported.** `record_spec_traceability` accepted whatever operation an agent claimed it had performed, and a spec task could be marked complete with a link that didn't actually resolve to anything. Traceability is now checked against git-classified file operations, and both recording a link and completing a task now fail outright, rather than silently succeeding, when a link doesn't resolve.
- **MDX content relied on one brittle, general-purpose block.** Plans and specs could only represent visual content through a single `<ModelDiagram>` component, regardless of what was actually being shown. It's been replaced with purpose-built blocks - DataModel, ApiEndpoint, Wireframe, Canvas and ModelChanges - so a plan can show an actual data model or API surface instead of describing one in prose.
- **Connecting an AI agent meant one generic prompt.** The old `.mcp.json` notification didn't say which agent it was about or what was actually connected. The Intent MCP tab now lists every supported agent individually, with its own live connection status and a one-click Connect.

This release also includes a long list of smaller fixes and improvements, covered further down.
